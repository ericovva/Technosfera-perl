...
sub tokenize {
	...
	$expr =~s/\s//g;
	if ($expr =~/([^0-9\+\-\(\)\^\*\/\.e])/) {
		die "Not permissible characters: ".$1
	}
	...
	for (my $i = 1; $i < scalar(@res_); $i++) {
		if (isDigit($res_[$i])) {
			...
			if ($flag) {
				die "Wrong syntax of DEC number";
			}
			$flag = 1;
	...
	for (my $i = 1; $i < scalar(@res) - 1; $i++) {
		...
		if ($res[$i] !~/[^\+\-\*\/\^]/) {
			if ($res[$i - 1] =~/([^\d+\)\.])/) {
				die "Wrong operand: \"".$1."\" before ".$res[$i];
			}
			if ($res[$i + 1] =~/([^\d+|\(|U\+|U\-|\.])/) {
				die "Wrong operand: \"".$1."\" after ".$res[$i];
			}
		}
